...
public class EventsRecorder {
    ...
    public void init(Supplier<Vertx> vertxSupplier, List<EventConsumerInfo> consumers,
            List<Class<?>> eventTypes, ShutdownContext shutdown) {
        vertx = vertxSupplier.get();
        messageConsumers = new CopyOnWriteArrayList<>();

        registerCodecs(eventTypes);
    ...
    private void registerCodecs(List<Class<?>> eventTypes) {
        EventBus eventBus = vertx.eventBus();
        for (Class<?> eventType : eventTypes) {
            try {
                eventBus.registerDefaultCodec(eventType,
                        new LocalEventBusCodec("quarkus_events_codec_" + eventType.getName()));
            } catch (IllegalStateException e) {
                // Codec already registered (e.g., by quarkus-vertx for the same type), skip
                LOGGER.debugf("Codec already registered for %s", eventType.getName());
            }
        }
    }
    ...
    private void registerConsumers(List<EventConsumerInfo> consumers) {
        ...
        int totalRegistrations = consumers.stream().mapToInt(c -> c.getAddresses().size()).sum();
        CountDownLatch latch = new CountDownLatch(totalRegistrations);
        ...
        for (EventConsumerInfo info : consumers) {
            ...
            for (String address : info.getAddresses()) {
                // Each consumer registration gets its own event loop context
                // to avoid serialization of published messages
