...
### 3.2 Recommended Approach: Multi-Address Registration (Build-Time Fan-Out)

At build time, Quarkus knows all concrete event types and all consumers via the Jandex index. For each consumer observing type `T`:

1. Find `T` itself
2. Find all known subclasses of `T` (`index.getAllKnownSubclasses(T)`)
3. Find all known implementors if `T` is an interface (`index.getAllKnownImplementors(T)`)
4. Register the consumer on the Vert.x address for `T` **and every discovered subtype**
...
@OnEvent void onPayment(PaymentReceived e) { ... }  // consumer C
```

Build-time registration:

| Consumer | Registered on addresses |
|---|---|
| A (`DomainEvent`) | `__qx_event__/OrderCreated`, `__qx_event__/OrderCancelled`, `__qx_event__/PaymentReceived`, `__qx_event__/DomainEvent` |
| B (`OrderCreated`) | `__qx_event__/OrderCreated` |
| C (`PaymentReceived`) | `__qx_event__/PaymentReceived` |

Sending `new OrderCreated()`:
- Address: `__qx_event__/OrderCreated`
- **Publish:** Both A and B receive it ✓
- **Send (P2P):** Round-robin between A and B ✓
- **Request:** One of A or B replies ✓

Sending `new PaymentReceived()`:
- Address: `__qx_event__/PaymentReceived`
- Both A and C receive/participate ✓

### 3.3 Advantages of This Approach

- **Leverages Vert.x's native routing** — round-robin and broadcast work naturally without a custom dispatch layer
- **Zero runtime overhead** for type resolution — all wiring happens at build time
- **Fits Quarkus's build-time philosophy** — no reflection, no runtime type hierarchy walking
- **Supports GraalVM native** — no dynamic class loading

### 3.4 Index Selection: Bean Archive vs Combined Index

Quarkus maintains multiple Jandex indexes and the processor must use the right one for each query:

- **Bean archive index** (`BeanArchiveIndexBuildItem`) — contains only classes from bean archives. Use this to discover `@OnEvent` consumer methods (they must be on CDI beans).
- **Combined index** (`CombinedIndexBuildItem`) — contains all application classes including non-bean archives (third-party libraries, modules without `beans.xml`, etc.). Use this for **subtype discovery** (`getAllKnownSubclasses`, `getAllKnownImplementors`), because event types and their subtypes may reside outside bean archives.

This matches the existing pattern in `EventBusCodecProcessor`, which uses the bean archive index to find `@ConsumeEvent` annotations but the combined index to find subclasses (line 142).

Getting this wrong would cause silent fan-out gaps — a consumer of `DomainEvent` would miss subtypes that live in a non-bean archive.

### 3.5 Limitations & Mitigations

| Limitation | Mitigation |
|---|---|
| Only works for types known at build time | Acceptable for Quarkus-native API; programmatic consumers handle dynamic cases (see §6) |
| Consumer of `Object` registers on every event address | Build-time validation: **error** if someone observes `Object` or `java.io.Serializable` directly; allow with explicit opt-in annotation |
| N subtypes × M supertype-consumers = N×M registrations | In practice, event type hierarchies are small; monitor with build-time reporting |
| New event types added by extensions after build | Extension integration points needed (see §9) |
...
When `send(new Dog())` hits both a `Dog` consumer and an `Animal` consumer (both registered on `__qx_event__/Dog`), Vert.x round-robins between them.
...
**Address mapping:** Qualifiers are appended to the address string to form a unique routing key:
- `Order` without qualifier → `__qx_event__/com.example.Order`
- `Order` with `@Premium` → `__qx_event__/com.example.Order#com.example.Premium`
- Multiple qualifiers must be **deterministically ordered** before concatenation to guarantee that senders and consumers produce the same address. Qualifiers are sorted lexicographically by their fully qualified annotation class name (e.g., `#com.example.Express#com.example.Premium`). If qualifier annotations carry members (e.g., `@Priority(5)`), the canonical form must include member values in a stable representation (e.g., `com.example.Priority(value=5)`).
...
**Build-time impact:** The fan-out set must be computed per qualifier combination, not just per type. This multiplies the number of address registrations but remains bounded since qualifier sets are known at build time. Qualifier support is included in the MVP — not deferred to a later version.
...
EventConsumerRegistration reg = orderEvent.consumer(order -> {
    // handle order
});
...
Programmatic consumers registered at runtime can't benefit from build-time subtype scanning. Options:

1. **Exact type only** — programmatic consumers only receive events of the exact registered type, no subtype matching. Simple and predictable.
2. **Runtime type closure** — when registering programmatically for type `T`, compute the subtype set at runtime using reflection and register on all subtype addresses. Possible but:
   - Requires knowing all subtypes at registration time (may not be complete in native mode)
   - Doesn't work well with GraalVM if classes aren't registered for reflection
3. **Hybrid** — register on exact type address, plus maintain a runtime registry that the build-time dispatcher consults

**Recommendation:** Option 1 (exact type only) for simplicity. Document the limitation. If subtype matching is needed, use a declarative `@OnEvent` consumer instead.
...
A new `UnifiedEventBusProcessor` (deployment module) performs:

```
                                    ┌─────────────────────┐
   Jandex Index ──────────────────►│  1. Scan @OnEvent    │
                                    │     methods          │
                                    └─────────┬───────────┘
                                              │
                                    ┌─────────▼───────────┐
                                    ...
                                    └─────────┬───────────┘
                                              │
                                    ┌─────────▼───────────┐
   Jandex: getAllKnownSubclasses ──►│  3. Compute subtype  │
   Jandex: getAllKnownImplementors  │     fan-out set      │
                                    └─────────┬───────────┘
                                              │
                                    ┌─────────▼───────────┐
                                    │  4. Generate address │
...
**Step 1-2: Scan and extract** — similar to `VertxProcessor.collectEventConsumers()`, iterate over beans with `@OnEvent` methods. Use `BeanRegistrationPhaseBuildItem` and the Jandex `AnnotationStore`.

**Step 3: Subtype fan-out** — for each observed type `T`:
```java
Set<DotName> subtypes = new HashSet<>();
subtypes.add(T);
subtypes.addAll(index.getAllKnownSubclasses(T));
if (isInterface(T)) {
    subtypes.addAll(index.getAllKnownImplementors(T));
}
// For each subtype, generate a Vert.x address and register the consumer
```

**Step 4: Generate registrations** — produce `EventConsumerBuildItem` records consumed by the runtime recorder.

**Step 5: Codecs** — reuse existing `EventBusCodecProcessor` logic. For local-only delivery, `LocalEventBusCodec` handles arbitrary types by passing object references.

**Step 6: Validate:**
- Error if `@OnEvent` parameter is `Object`, `Serializable`, or similar overly broad types
- Warning if multiple reply-capable consumers match the same concrete event type
- Error if `@OnEvent` method has unsupported signature
...
| `eventBus.consumer("address")` | `quarkusEvent.consumer(handler)` |
...
// Programmatic consumer registration (exact type only, see §6)
EventConsumerRegistration reg = orderEvent.consumer(order -> {
    // handle order
});
